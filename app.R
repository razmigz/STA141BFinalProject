...
pokemon.names <- link %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 10)
...
gen1.moves <- gen1.moves %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7) %>% tibble()

gen2.moves <- read_html("https://pokemondb.net/move/generation/2")
gen2.moves <- gen2.moves %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7) %>% tibble()

gen3.moves <- read_html("https://pokemondb.net/move/generation/3")
gen3.moves <- gen3.moves %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7) %>% tibble()

gen4.moves <- read_html("https://pokemondb.net/move/generation/4")
gen4.moves <- gen4.moves %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7) %>% tibble()

gen5.moves <- read_html("https://pokemondb.net/move/generation/5")
gen5.moves <- gen5.moves %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7) %>% tibble()

gen6.moves <- read_html("https://pokemondb.net/move/generation/6")
gen6.moves <- gen6.moves %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7) %>% tibble()

gen7.moves <- read_html("https://pokemondb.net/move/generation/7")
gen7.moves <- gen7.moves %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7) %>% tibble()
#combine all moves by generation into one to use later in a dropdown menu
all.moves <- list(gen1.moves, gen2.moves, gen3.moves, gen4.moves, gen5.moves,
     gen6.moves, gen7.moves) %>% reduce(left_join)
################################################################################
#web scrape for items
items.link <- read_html("https://pokemondb.net/item/all")
all.items <- items.link %>% html_nodes("a.ent-name") %>% html_attr("href") %>% substring(., 7)
...
all.items <- all.items[-c(6, 14, 26, 34, 53:56, 58, 59, 61, 64, 71, 84, 92, 94, 99:101, 103, 109, 111, 129, 137, 139, 141, 146, 
    ...
    tabsetPanel(
      ...
      tabPanel("Pokemon Information",
        ...
        mainPanel(
          ...
          selectInput("pokemon", "Pick a Pokemon to see its stats and what abilities it gets:",
...
server <- function(input, output, session) {
  ...
  ##tab 1
    ...
    stat.table <- chosen.pok$stats %>% select(base_stat) %>% cbind(c("Speed", "Special Defense", "Special Attack", 
                                                                     "Defense", "Attack", "HP")) %>% tibble() %>% 
      #make it easier to read by putting stat name before content
      select(base_stat) %>% rename("Base Stat Value" = "base_stat")
    
    stat.table <- cbind(stat.name, stat.table) %>% tibble() %>% rename("Stat Name" = "stat.name")
    stat.table
  ...
  output$plt <- renderPlot({
    ...
    bst.label <- sprintf(
      ...
      input$pokemon,
      bst)
    stat.plot <- ggplot(stat.table, aes(stat.name, `Base Stat Value`)) + geom_bar(stat = "identity", col = "springgreen4",
                                                                                  fill = "thistle2") +
      xlab("Stat Name") + ggtitle(title) + labs(caption = bst.label) +
  ...
  output$type <- renderPrint({
    ...
    if (length(chosen.pok$types$type$name) == 1){
      ...
      cat(toTitleCase(chosen.pok$name), "is a", toTitleCase(chosen.pok$types$type$name[1]), "and",
  ...
  output$Abilities <- renderPrint({
    ...
    if ((length(chosen.pok$abilities$ability$name)) == 1){
      cat(toTitleCase(chosen.pok$name), "has the ability", toTitleCase(chosen.pok$abilities$ability$name))
        ...
        cat(c(toTitleCase(chosen.pok$name), paste0("can have the following abilites: ", toTitleCase(ability1), " and ", 
                                                   toTitleCase(ability2))))
      } else{
        ability1 <- abilities[1]
        ability2 <- abilities[2]
        ability3 <- abilities[3]
        cat(c(toTitleCase(chosen.pok$name), paste0("can have the following abilites: ", toTitleCase(ability1), ", ",
  ...
  if (move.info$damage_class$name == "status"){
    cat(toTitleCase(name.of.move), "is a", toTitleCase(move.info$type$name), "type move",
        "\n The damage type of", toTitleCase(name.of.move), "is", move.info$damage_class$name,
        "\n ", toTitleCase(name.of.move), "does not do damage",
        "\n", paste0(toTitleCase(name.of.move)), "can be used", move.info$pp, "times until there are no power points left",
  ...
  output$Item <- renderPrint({
    ...
    if (items.content$cost == 0){
      cat(toTitleCase(items.content$name), "is not for sale!",
          toTitleCase(items.content$name), "does the following:\n", items.content$effect_entries$short_effect)
    } else{
      cat("A(n)", toTitleCase(items.content$name), "costs", items.content$cost, "pokedollars.\n", 
          toTitleCase(items.content$name),
          "does the following:", items.content$effect_entries$short_effect)
    }
  })
